...
function wp_rp_generate_related_posts_list_items($related_posts, $selected_related_posts) {
	...
	for ($i = 0; $i < $limit; $i++) {
		...
		$related_post_raw = get_post( $related_post->ID );
		$related_post_class = implode(' ', get_post_class('', $related_post->ID));
		$a = get_userdata( $related_post_raw->post_author );
		$a = sprintf( // stolen from function the_author_posts_link()
			'<a href="%1$s" title="%2$s" rel="author">%3$s</a>',
			esc_url( get_author_posts_url( $a->ID, $a->data->display_name ) ),
			esc_attr( sprintf( __( 'Posts by %s' ), $a->data->display_name ) ),
			$a->data->display_name
		);
		$related_post_author = apply_filters( 'the_author_posts_link', $a );

		$output .= '<li class="blog-entry ' . $related_post_class . '"' . $data_attrs . '>';

		$post_url = property_exists($related_post, 'post_url') ? $related_post->post_url : get_permalink($related_post->ID);

		$img = wp_rp_get_post_thumbnail_img($related_post, $image_size);
		if ($img) {
			$output .=  '<a href="' . $post_url . '" class="wp_rp_thumbnail"><div class="blog-entry-thumbnail">' . $img . '</div></a>';
		}
		...
		// $a = get_the_category_list('', '', $related_post->ID);
		// echo $related_post->ID . '#' . $related_post->ID . '#' . $a;
		// echo '</textarea>';

		$output .= '<div class="entry-text"><h2 class="title"><a href="' . $post_url . '" class="wp_rp_title">' . wptexturize($related_post->post_title) . '</a></h2>' . '<ul class="entry-meta"> by ' . $related_post_author . '<div class="cat">' . get_the_category_list('', '', $related_post->ID) . '</div></ul></div>';
